...
public class UILayerManager : MonoBehaviour
{
	...
	/// <summary>
	/// 일시정지 패널의 "메뉴" 버튼에 연결하세요.
	/// 게임을 중단하고 메인메뉴 씬으로 돌아갑니다.
	///
	/// GameManager는 DontDestroyOnLoad이므로 씬이 바뀌어도 그대로
	/// 유지됩니다. 메인메뉴로 돌아갈 때는 상태를 MainMenu로 바꾸고,
	/// 스택/일시정지 플래그를 초기화한 뒤 씬을 전환합니다.
	/// (UILayerManager 자신은 DontDestroyOnLoad가 아니므로 씬 전환과
	/// 함께 자동으로 파괴되고, 새 씬의 UILayerManager가 새로 생성됩니다.)
	///
	/// Unity Button의 OnClick에서 바로 연결할 수 있는 파라미터 없는 버전입니다.
	/// 메인메뉴 씬 이름이 다르면 ReturnToMainMenu(string)을 직접 호출하세요.
	/// </summary>
	public void ReturnToMainMenu()
	{
		ReturnToMainMenu("01_MainMenu");
	}
